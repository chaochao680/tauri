...
给组件绑定菜单，控制菜单显隐的触发方式为长按或右键点击，弹出的菜单项需自定义。若需通过代码逻辑控制菜单显隐，请使用bindContextMenu。
...
symbolIcon	SymbolGlyphModifier	否	是	
...
2. 作为bindContextMenu或bindContextMenuWithResponse入参时，默认效果为菜单跟随点击位置弹出。

3. 作为bindContextMenu入参时，默认值为Placement.BottomLeft。
...
preview	MenuPreviewMode | CustomBuilder	否	是	
长按悬浮菜单或使用bindContextMenu显示菜单的预览内容样式，可以为目标组件的截图，也可以为用户自定义的内容。
...
previewAnimationOptions	ContextMenuAnimationOptions	否	是	
控制长按预览的显示效果。
...
previewBorderRadius	BorderRadiusType	否	是	
设置预览图边框圆角半径。
...
previewScaleMode	PreviewScaleMode	否	是	
预览图缩放方式。
...
availableLayoutArea	AvailableLayoutArea	否	是	
...
aboutToAppear	() => void	否	是	
菜单显示动效前的事件回调。

元服务API： 从API version 12开始，该接口支持在元服务中使用。

aboutToDisappear	() => void	否	是	
菜单退出动效前的事件回调。
...
borderRadius	Length | BorderRadiuses | LocalizedBorderRadiuses	否	是	
设置菜单的边框圆角半径。
...
hapticFeedbackMode	HapticFeedbackMode	否	是	
菜单弹出时振动效果。
...
outlineWidth	Dimension | EdgeOutlineWidths	否	是	
设置菜单边框外描边宽度。
...
outlineColor	ResourceColor | EdgeColors	否	是	
设置菜单边框外描边颜色。
...
keyboardAvoidMode	MenuKeyboardAvoidMode	否	是	
设置菜单是否避让软键盘。
...
minKeyboardAvoidDistance	LengthMetrics	否	是	
设置菜单避让软键盘的最小距离。
...
bindMenu	Placement.BottomLeft
bindContextMenu	Placement.Top
bindContextMenu	Placement.BottomLeft
bindContextMenuWithResponse	Placement.Top
MenuPreviewMode
Phone
PC/2in1
Tablet
...
hoverScale	AnimationRange<number>	否	是	
...
hoverScaleInterruption	boolean	否	是	
