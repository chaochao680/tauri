...
| 测试 | 改动前 URL | 改动后 URL | 断言变化 |
|------|-----------|-----------|---------|
| GET | `https://httpbin.org/get` | `http://localhost:3003/get` | 移除 data.url 断言，仅验证 status=200 |
| POST | `https://httpbin.org/post` | `http://localhost:3003/post` | `data.json.test` → `resp.text() === body` |
| PUT | `https://httpbin.org/put` | `http://localhost:3003/put` | `data.json.update` → `resp.text() === body` |
| DELETE | `https://httpbin.org/delete` | `http://localhost:3003/delete` | 不变 |
| custom headers | `https://httpbin.org/headers` | `http://localhost:3003/headers` | `data.headers[...]` → `resp.headers.get(...)` |
| error handling | `https://httpbin.org/status/404` | `http://localhost:3003/status/404` | 不变 |
| JSON parse | `https://httpbin.org/json` | 不变（加 retryFetch） | 不变 |
| HTTPS/rustls-tls | `https://httpbin.org/get` | 不变（加 retryFetch） | 不变 |
