...
| Tauri/tao 接口 | OHOS 实现方式 | 参数格式 | 说明 |
|---------------|--------------|---------|------|
| `WindowAttributes.transparent: bool` | NAPI config.transparent → WindowManager → `setWindowBackgroundColor('#00000000')` | `boolean` | 默认 `false` |
| `WindowAttributes.background_color: Option<RGBA>` | Rust RGBA→`0xAARRGGBB` → NAPI config.backgroundColor → `setWindowBackgroundColor('#AARRGGBB')` | number `0xAARRGGBB` | 默认 `None` |
| `Window::set_background_color(Option<RGBA>)` | NAPI `setWindowBackgroundColor(windowId, color)` → WindowManager → `win.setWindowBackgroundColor()` | `(i64, u32)` | 运行时更新 |
| `window.Window.setWindowBackgroundColor()` | OHOS Window API 9+ | `string` (`#AARRGGBB`) | 主窗口和子窗口均支持 |
