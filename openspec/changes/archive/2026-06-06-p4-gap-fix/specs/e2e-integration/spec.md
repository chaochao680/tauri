...
|---------------|--------------|------|------|
| `WindowCreateParams` | Rust 结构体 → NAPI config 对象 | `{name, type, windowId, decorations, transparent, backgroundColor}` | 统一参数传递 |
| `initMainWindow(config)` | NAPI → ArkTS WindowManager | `MainWindowConfig` | 主窗口属性统一初始化 |
| `setWindowDecorations(windowId, bool)` | NAPI → ArkTS | `(i64, boolean)` | 运行时 decorations 切换 |
| `setWindowBackgroundColor(windowId, u32)` | NAPI → ArkTS | `(i64, u32)` | 运行时背景色更新 |
