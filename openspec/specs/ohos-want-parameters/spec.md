# ohos-want-parameters Specification

## Purpose
ArkTS→NAPI→Rust 链路传递 want.parameters 的数据格式与接口规范。
## Requirements
...
`WindowStageEventCallback.onNewWant` 的类型签名 SHALL 从 `(uri: string) => void` 扩展为 `(data: NewWantData) => void`，其中 `NewWantData` 为专用接口，包含 `uri: string` 和 `parametersJson: string` 两个字段。

#### Scenario: 新接口兼容性
- **WHEN** lifecycle module 注册 `onNewWant` 回调
- **THEN** 回调 SHALL 接收一个 `NewWantData` 对象参数，含 `uri` 和 `parametersJson` 字段
