# ohos-single-instance Specification

## Purpose
OHOS 平台上 single-instance plugin 的实现规范，定义 init/destroy 接口及 onNewWant callback 触发行为。
## Requirements
...
- **THEN** callback SHALL 被调用，`args` 包含系统注入的 want.parameters JSON（空 URI 被过滤），`cwd` = `""`。注意：即使不传自定义参数，OHOS 系统也会注入 `isShellCall`、`moduleName` 等字段到 `want.parameters`
